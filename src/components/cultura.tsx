...
const Cultura: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (titleRef1.current) {
            ...
            circleType1.radius(220).dir(1); // Aplicar el radio y la dirección a cada elemento
        }

        if (titleRef2.current) {
            const circleType2 = new CircleType(titleRef2.current);
            circleType2.radius(300).dir(1); // Diferente radio y dirección si lo deseas
        }

        if (titleRef3.current) {
            const circleType3 = new CircleType(titleRef3.current);
            circleType3.radius(220).dir(1); // Diferente radio y dirección si lo deseas
        }
        return () => {
            removeEvents()
        }
        ...
        <Box display='flex' flexDirection='column' justifyContent='center' justifyItems='center' onWheel={handleWheelEvent}>
            <Box height='100vh' display="flex" gap="10px" justifyContent='center' id='section10'>
                <Box position='absolute' width='60%' marginTop='50px'>
                    <Heading fontFamily='Mate SC' as='h1' size='xl'>Explora la Historia de la Aldea de San Nicolás</Heading>
                    <Text fontFamily='Quicksand' marginTop='2vh'>Descubre los momentos clave que han dado forma a la Aldea de San Nicolás con nuestro Eje Cronológico Interactivo. Desde sus orígenes hasta la actualidad, recorre los eventos más importantes que marcaron su desarrollo.</Text></Box>
                <Box display='flex' justifyContent='center' alignItems='center'>
                    ...
                    <MotionBox textAlign='center' className=' ejeIinfoView' position='absolute' width='60%' marginTop='50vh' fontFamily='Lora' fontSize='lg'>Haz clic en cada etapa y viaja por el tiempo para conocer más sobre nuestra historia.</MotionBox>
                </Box></Box>
            <Box display='flex' flexDirection='column' justifyContent='center' gap='20' height='100vh' id='section12'>
                <Box display='flex' flexDirection='column' alignItems='center' gap='5'>
                    <Heading fontFamily='Mate SC' fontSize='7xl'>Fiestas y tradiciones</Heading>
                    <Text width='800px' fontFamily='Lora' fontSize='lg'>La Aldea de San Nicolás es conocida por sus vibrantes fiestas y tradiciones que mantienen viva la cultura local:
                ...
                <Box display='flex' justifyContent='space-evenly' alignItems='center' gap='5' >
                    <Box onClick={() => { onOpen(); setModalInfo(fiestasInfo.laRama) }} className='circleParty' display='flex' justifyContent='center' flexDirection='column' alignItems='center' bg='white' >
                        <div style={{ fontSize: '30px', fontFamily: 'Playwrite CU' }} ref={titleRef1} className='titleFiesta'>La Rama</div>
                        <Box className='circleParty2' width='100%' height='100%' display='flex' justifyContent='center' alignItems='center'>
                        <Text bg='transparent' fontFamily='EB Garamond, serif' color='white' fontSize='9xl' position='absolute' textShadow='-2px -2px 0 black,4px -2px 0 black,-2px 4px 0 black,4px 4px 0 black;'>9</Text>
                        <Image className='imgFiestas' width='100%' height='100%' src='..\src\assets\circularPlayaPuerto\1bc75dd9-7f79-4910-a31f-3c9f1ffb6745_16-9-aspect-ratio_default_0.jpg'></Image></Box>
                    </Box>
                    <Box className='circleParty' onClick={() => { onOpen(); setModalInfo(fiestasInfo.laRomeria) }} bg='white' display='flex' flexDirection='column' justifyContent='center' alignItems='center' >
                        <Box>
                            <div style={{ fontSize: '30px', fontFamily: 'Playwrite CU' }} ref={titleRef2} className='titleFiesta'>La Romeria</div></Box>
                            <Box className='circleParty2' width='100%' height='100%' display='flex' justifyContent='center' alignItems='center'>
                        <Text bg='transparent' fontFamily='EB Garamond, serif' color='white' fontSize='9xl' position='absolute' textShadow='-2px -2px 0 black,4px -2px 0 black,-2px 4px 0 black,4px 4px 0 black;'>10</Text>
                        <Image className='imgFiestas' width='100%' height='100%' src='..\src\assets\circularPlayaPuerto\Romeria-La-Aldea-SN.jpg'></Image>
                        </Box>
                    </Box>
                    <Box className='circleParty' onClick={() => { onOpen(); setModalInfo(fiestasInfo.elCharco) }} bg='white' display='flex' flexDirection='column' justifyContent='center' alignItems='center' zIndex='1'>
                        <Box>
                            <div style={{ fontSize: '30px', fontFamily: 'Playwrite CU' }} ref={titleRef3} className='titleFiesta'>El Charco</div></Box>
                        <Box className='circleParty2' width='100%' height='100%' display='flex' justifyContent='center' alignItems='center'>
                            <Text bg='transparent' fontFamily='EB Garamond, serif' color='white' fontSize='9xl' position='absolute' textShadow='-2px -2px 0 black,4px -2px 0 black,-2px 4px 0 black,4px 4px 0 black;'>11</Text>
                            <Image width='100%' height='100%' borderRadius='50%' src='..\src\assets\circularPlayaPuerto\fiestas charco-k25B-U1701117236276O0E-1248x770@Canarias7 (1).webp'></Image></Box>
                    </Box>
                </Box>
            </Box>
            <Modal isOpen={isOpen} onClose={onClose} size={'xl'}>
                <ModalOverlay />
                <ModalContent>
                    <ModalHeader paddingBottom='0' fontSize='4xl'>{modalInfo.titulo}</ModalHeader>
                    <ModalCloseButton />
                    <ModalBody>
